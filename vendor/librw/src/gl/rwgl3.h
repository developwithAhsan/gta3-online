...
// Under Emscripten, WebGL headers are provided by the Emscripten toolchain itself.
// glad must not be included; instead include the GLES2/GL2 + GLFW emscripten port.
#include <GLES3/gl3.h>
#include <GLFW/glfw3.h>

// GLES3/gl3.h only exposes core WebGL2/GLES3 tokens. librw's GL3 backend also
// references these desktop-GL/extension tokens for capability paths that are always
// disabled under Emscripten (see gl3Caps.dxtSupported/astcSupported), so the values
// are never actually sent to the driver -- plain compile-time compatible values
// (taken from the real EXT_texture_compression_s3tc / EXT_texture_filter_anisotropic
// specs) are enough to satisfy the compiler.

...
// WebGL2 only accepts GLSL ES 3.00 (`#version 300 es`); GLSL ES 3.10
// (shaderDecl310es above) is tied to OpenGL ES 3.1 and WebGL2 rejects it outright.
// None of librw's GL3 shaders (vendor/librw/src/gl/shaders/) use anything beyond
// GLSL ES 3.00 (no compute/image-load-store/etc), so this loses nothing -- it's a
// separate constant, not a change to shaderDecl310es, specifically so real desktop
// GLES 3.1 platforms are unaffected. See docs/WEBGL_COMPATIBILITY.md.
...
#ifdef __EMSCRIPTEN__
	// Emscripten's GLFW3 port creates a WebGL context for *any* GLFW_CLIENT_API
	// request -- desktop GL or ES -- so glfwCreateWindow() above always succeeds on
	// the very first profiles[] entry ({GLFW_OPENGL_API, 3, 3}), and the gles/
	// glversion guess from `profiles[i]` ends up wrong (reports a desktop "OpenGL
	// 3.3" context that was never actually created). We know at compile time this
	// is always a WebGL2 context (USE_WEBGL2=1, src/CMakeLists.txt), equivalent to
	// OpenGL ES 3.0, so set the real values directly instead of trusting the
	// profile loop. This matters: it's what selects GLSL ES vs desktop GLSL shader
	// source below. See docs/WEBGL_COMPATIBILITY.md.
	gl3Caps.gles = true;
...
*/
#ifdef __EMSCRIPTEN__
	// No GLAD loader (and thus no extension-support flags) under Emscripten.
	// WebGL2 doesn't expose S3TC/ASTC compressed textures or anisotropic filtering
	// without an explicit (and here unqueried) WEBGL_* extension, so treat them as
	// unsupported.

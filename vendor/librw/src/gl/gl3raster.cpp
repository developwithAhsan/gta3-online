...
rasterCreateCamera(Raster *raster)
{
	...
	// WebGL2's glReadPixels() only guarantees GL_RGBA/GL_UNSIGNED_BYTE (or a second,
	// implementation-defined combo queried via GL_IMPLEMENTATION_COLOR_READ_*);
	// unlike desktop GL, it does not support arbitrary format conversion during
	// readback -- GL_RGB fails with GL_INVALID_OPERATION. rasterLock()'s
	// `case Raster::CAMERA:` below reads back as RGBA to match; keep format/bpp
	// here in sync with that so rasterToImage()'s format-based converter (see
	// raster.cpp/Raster::toImage) interprets the resulting bytes correctly. See
	// docs/WEBGL_COMPATIBILITY.md.
		...
		if(lockMode & Raster::LOCKREAD || !(lockMode & Raster::LOCKNOFETCH)){
			if(natras->isCompressed){
				if(natras->backingStore){
					...
					bindTexture(prev);
#else
					// glGetCompressedTexImage doesn't exist in WebGL/GLES, and
					// gl3Caps.dxtSupported is always false under Emscripten, so
					// natras->isCompressed should never be true here.
					assert(0 && "compressed texture readback without a backing store is not supported under Emscripten");
#endif
				}
...
//e = glGetError(); printf("GL err4 %x (%x)\n", e, natras->format);
				...
				bindTexture(prev);
#else
				// glGetTexImage doesn't exist in WebGL/GLES; gl3Caps.gles is always
				// true under Emscripten, so the branch above is taken instead.
				assert(0 && "glGetTexImage is not supported under Emscripten");
#endif
			}
		}
...
#ifdef __EMSCRIPTEN__
		// See the __EMSCRIPTEN__ branch in rasterCreateCamera() above: WebGL2
		// glReadPixels() only accepts GL_RGBA/GL_UNSIGNED_BYTE (GL_RGB fails with
		// GL_INVALID_OPERATION), so this reads 4 bytes/pixel where the native path
		// reads 3 -- raster->format/natras->bpp were set to match above.

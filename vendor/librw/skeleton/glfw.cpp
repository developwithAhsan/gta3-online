...
#ifdef __EMSCRIPTEN__
	// The blocking while(...) loop below never yields to the browser -- it would
	// either freeze the tab or (with no way to present a frame) never actually show
	// anything. Same fix as src/skel/glfw/glfw.cpp in the main re3 skeleton: hand
	// per-frame work to the browser's own rAF-driven loop instead. See
	// docs/WEBGL_COMPATIBILITY.md.
	static float lastTime = glfwGetTime()*1000;

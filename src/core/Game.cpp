...
// Phase 9 (docs/FIRST_BOOT.md): defined in src/skel/glfw/glfw.cpp.
...
{
#ifdef __EMSCRIPTEN__
	debug("[BOOT] ENTER CGame::InitialiseOnceBeforeRW\n");
#endif
	CFileMgr::Initialise();
...
#endif
#ifdef __EMSCRIPTEN__
	debug("[BOOT] EXIT CGame::InitialiseOnceBeforeRW\n");
#endif
	return true;
}
...
{
#ifdef __EMSCRIPTEN__
	debug("[BOOT] ENTER CGame::InitialiseOnceAfterRW\n");
#endif
#if GTA_VERSION > GTA3_PS2_160
	TheText.Load();
	DMAudio.Initialise();	// before TheGame() on PS2
#ifdef __EMSCRIPTEN__
	re3_SetBootStage(4); // [4] Audio initialized
#endif
	CTimer::Initialise();
...
#endif
#ifdef __EMSCRIPTEN__
	re3_SetBootStage(5); // [5] Game data loaded
	debug("[BOOT] EXIT CGame::InitialiseOnceAfterRW\n");
#endif
	return true;
}
...
{
#ifdef __EMSCRIPTEN__
	// Debugging instrumentation (menu->gameplay transition investigation):
	// this is the actual "STREAMING: request issued" point -- CGame::Initialise()
	// is what pulls in IDE/IPL/COL/models via CFileLoader (which itself reads
	// through CdStreamRead()/CdStreamSync(), see src/core/CdStreamPosix.cpp).
	debug("GAME: CGame::Initialise entered\n");
#endif
#ifdef GTA_PS2
	...
	CReferences::Init();
#ifdef __EMSCRIPTEN__
	debug("[BOOT] ENTER TheCamera.Init\n");
#endif
	TheCamera.Init();
	TheCamera.SetRwCamera(Scene.camera);
#ifdef __EMSCRIPTEN__
	debug("[BOOT] EXIT TheCamera.Init (camera=%p)\n", (void*)&TheCamera);
#endif
...
#ifdef __EMSCRIPTEN__
	debug("[BOOT] ENTER CWorld::Initialise\n");
#endif
	CWorld::Initialise();
#ifdef __EMSCRIPTEN__
	debug("[BOOT] EXIT CWorld::Initialise\n");
#endif
	POP_MEMID();
...
#ifdef __EMSCRIPTEN__
	debug("[BOOT] ENTER CTheScripts::StartTestScript+Process (first script tick)\n");
#endif
#ifdef PS2_MENU
...
#ifdef __EMSCRIPTEN__
	debug("[BOOT] EXIT CTheScripts::StartTestScript+Process\n");
#endif
...
#ifdef __EMSCRIPTEN__
	debug("GAME: CGame::Initialise completed -- STREAMING: completed\n");
#endif
	return true;
}

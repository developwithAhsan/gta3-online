...
#else
	// Pointer lock (browser mouse capture) is engaged/released entirely from
	// web/launcher.js via the standard Pointer Lock DOM API (canvas.
	// requestPointerLock()/document.exitPointerLock()) -- not through
	// glfwSetInputMode, which under Emscripten only implements GLFW_CURSOR_NORMAL
	// and GLFW_CURSOR_DISABLED (GLFW_CURSOR_HIDDEN is an unimplemented no-op, and
	// glfwGetInputMode's WASM import isn't even linkable in this Emscripten
	// version). Emscripten's shared Browser.pointerLock state (which
	// glfwGetCursorPos's delta tracking below actually reads) is updated by a
	// pointerlockchange listener Emscripten itself registers, so this works
	// correctly regardless of which side requested the lock. web/input.js releases
	// the lock itself when the menu becomes active (polling a tiny read-only
	// re3_IsMenuActive() getter) and on browser focus loss. See
	// docs/BROWSER_INPUT.md.
...
#ifdef __EMSCRIPTEN__
	// Debug-only: forces a constant "walk/drive forward" pad state for the
	// gameplay-stability investigation (docs/WASM_GAMEPLAY_STABILITY.md) --
	// this test harness has no real browser keyboard/mouse input, but the
	// reported crash needed the player actually moving (streaming/physics
	// exercised) to reproduce, not just standing still post-load. Injected
	// after the real Update(0) call so it isn't overwritten by (and doesn't
	// interfere with) the normal keyboard/mouse/joystick pipeline above.
	// Off by default; toggled at runtime via re3_DebugSetForceForward().
	extern bool g_re3DebugForceForward;
	if (g_re3DebugForceForward)
		GetPad(0)->NewState.LeftStickY = -128;
#endif
...
#ifdef __EMSCRIPTEN__
bool g_re3DebugForceForward = false;

extern "C" EMSCRIPTEN_KEEPALIVE void
re3_DebugSetForceForward(int enable)
{
	g_re3DebugForceForward = enable != 0;
}
#endif

...
// Phase 8 (docs/THREADING_WASM.md): no background thread at all under
// Emscripten. The original PS2/PC design used a dedicated worker thread here to
// hide real CD-ROM/HDD seek latency behind the main loop -- but re3's "disk" in
// the browser is a virtual filesystem (MEMFS/IDBFS, see docs/GAME_ASSETS.md)
// that's already resident in memory, so there is no real seek latency to hide.
// The pthread-based version below also doesn't work reliably under Emscripten
// regardless: named POSIX semaphores via sem_open() aren't implemented by
// Emscripten's pthread support and fail in CdStreamInitThread(), which used to
// surface as an assert on every startup. CdStreamRead() below calls this
// directly and synchronously on the calling (main) thread instead of handing
// the read to a worker; CdStreamSync()'s wait loop then sees nSectorsToRead
// already at 0 and returns immediately without ever touching a semaphore.
static void
CdStreamPerformReadSync(CdReadInfo *pChannel)
{
	...
	if ( pChannel->nStatus == STREAM_NONE )
	{
		...
		size_t requested = (size_t)pChannel->nSectorsToRead * (size_t)CDSTREAM_SECTOR_SIZE;
		lseek(pChannel->hFile, (size_t)pChannel->nSectorOffset * (size_t)CDSTREAM_SECTOR_SIZE, SEEK_SET);
		ssize_t got = read(pChannel->hFile, pChannel->pBuffer, requested);
		if (got == -1) {
			pChannel->nStatus = pChannel->nSectorsToRead == 0 ? STREAM_WAITING : STREAM_ERROR;
			CDTRACE("[CDSTREAM] FAIL offset=%u sectors=%u requested=%zu result=error(errno=%d)",
				pChannel->nSectorOffset, pChannel->nSectorsToRead, requested, errno);
		} else if ( requested != 0 && (size_t)got != requested ) {
			// Short read: the underlying read() succeeded (no -1) but returned
			// fewer bytes than asked for. The original threaded implementation
			// never checked for this either (it treated any non -1 result as
			// success) -- called out explicitly here rather than silently
			// carried over, since a short read leaves the tail of pBuffer
			// with stale/uninitialised data that CStreaming would then treat
			// as valid model/texture data.
			pChannel->nStatus = STREAM_ERROR;
			CDTRACE("[CDSTREAM] SHORT READ offset=%u sectors=%u requested=%zu result=%zd",
				pChannel->nSectorOffset, pChannel->nSectorsToRead, requested, got);
		} else {
...
{
#ifdef __EMSCRIPTEN__
	gCdStreamThreadStatus = 0; // "not shut down" -- CdStreamGetStatus() checks this
	debug("Streaming reads run synchronously on the main thread (no background thread) under Emscripten\n");
	return;
#else
	int status;
#ifndef ONE_THREAD_PER_CHANNEL
...
CdStreamInit(int32 numChannels)
{
	...
	{
#ifdef __EMSCRIPTEN__
		// This is the first hard dependency on real GTA III data in the startup path
		// (see docs/GAME_ASSETS.md): every other missing file is reported by
		// casepath() as "Missing game asset: <path>" too, but this one is opened
		// directly (not through CFileMgr), so it needs its own explicit message.
		// Kept Emscripten-only so native builds' output is unchanged.
		CDTRACE("Missing game asset: models/gta3.img (relative to the current directory; can't get filesystem info)");
#else
...
{
#ifdef __EMSCRIPTEN__
	// No thread was ever created (see CdStreamInitThread()) -- just the cleanup
	// CdStreamThread()'s own shutdown tail would otherwise do for the native path.
...
#ifdef __EMSCRIPTEN__
	CdStreamPerformReadSync(pChannel);
#elif !defined ONE_THREAD_PER_CHANNEL

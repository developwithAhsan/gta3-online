...
char* casepath(char const* path, bool checkPathFirst)
{
    ...
    while (c = strsep(&p, "/\\"))
    {
        ...
        if (!e)
        {
            ...
            // Report the full requested path, not just the failing path component --
            // "Missing game asset: <path>" is a specific, greppable message instead of
            // a generic "File failed", so a missing GTA III install/asset is obvious
            // in the log rather than looking like an engine bug. See docs/GAME_ASSETS.md.
            // Kept Emscripten-only so native builds' output is unchanged.
            printf("Missing game asset: %s (looked for \"%s\" under the current directory)\n", path, c);
#else

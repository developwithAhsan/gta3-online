...
// Per-atomic render-callback plugin (see the full explanation further down,
// near RpAtomicSetRenderCallBack) -- registered from RwEngineInit() below,
// which needs these declared first.
static int32 re3RenderCBPluginOffset;
static void *re3RenderCBConstructor(void *object, int32 offset, int32 size);
static void *re3RenderCBCopy(void *dst, void *src, int32 offset, int32 size);
...
RwBool RwEngineInit(RwMemoryFunctions *memFuncs, RwUInt32 initFlags, RwUInt32 resArenaSize) {
	...
	// Registered here (rather than lazily on first use) because this runs
	// before any Atomic can possibly exist, guaranteeing every atomic ever
	// created has this plugin slot. See RpAtomicSetRenderCallBack above.
	// ID is a private tag for this plugin only -- it's never streamed
	// (no registerPluginStream call), so it just needs to not be 0; it
	// doesn't need to follow re3's own MAKECHUNKID/vendor-ID scheme, which
	// this low-level shim file has no access to (defined in core/common.h).
	re3RenderCBPluginOffset = Atomic::registerPlugin(sizeof(RpAtomicCallBackRender),
		0x52334352 /* 'R3CR' */,
		re3RenderCBConstructor, nil, re3RenderCBCopy);
	return true;
}
...
// re3's RpAtomicCallBackRender (RpAtomic*(*)(RpAtomic*), matching the original
// RenderWare API re3 was written against) and librw's actual Atomic::RenderCB
// (void(*)(Atomic*)) differ in return type. Reinterpret-casting between them
// (the auto-generated code below used to do this directly, hence the
// "illegal cast" this comment used to flag) happens to work on native x86/x64
// -- callers simply don't read an unused return register -- but traps under
// WebAssembly, whose indirect-call tables strictly validate the full function
// signature ("RuntimeError: function signature mismatch"). This only
// reproduces the first time an atomic actually renders (RpAtomicRender ->
// atomic->render() -> the stored renderCB), which under Emscripten didn't
// happen until real gameplay got past the loading screen.
//
// A first fix here used a single shared variable to remember "the last
// callback passed to RpAtomicSetRenderCallBack" and had the trampoline call
// through that. That's wrong: re3 registers many *distinct* callbacks this
// way for different atomics at once -- RenderWheelAtomicCB for vehicle
// wheels, RENDERCALLBACK (PreInstanceRenderCB/DefaultRenderCB_pushid) for
// ordinary objects, RenderObjNormalAtomic, etc. (see VisibilityPlugins.h) --
// so once more than one kind had ever been registered, whichever atomic
// happened to render *after* a different atomic's registration would run
// through the *wrong* callback. Confirmed live: CVisibilityPlugins::
// GetAtomicModelInfo() returned nil for wheel atomics whose own creation-time
// diagnostic had logged a correct, non-nil modelInfo moments earlier --
// consistent with a non-wheel atomic being misrouted into
// RenderWheelAtomicCB, which unconditionally dereferences that modelInfo
// pointer (no nil check) and crashed with "memory access out of bounds".
// This reproduced specifically once real gameplay time passed (new vehicles
// streaming in registered new/different callbacks), not from raw tick count,
// which is why it never showed up in short/synchronous test runs.
//
// The real fix needs genuine per-atomic storage. Rather than reach into
// re3's own VisibilityPlugins module (a backwards dependency for this
// low-level RW-compatibility shim), this registers its own tiny librw atomic
// plugin -- exactly the mechanism RenderWare/librw provides for exactly this
// need -- at RwEngineInit(), which runs before any Atomic can possibly exist,
// so every atomic ever created has this slot.

static void*
re3RenderCBConstructor(void *object, int32 offset, int32)
{
...
static void*
re3RenderCBCopy(void *dst, void *src, int32 offset, int32)
{
...
static void
re3AtomicRenderCBTrampoline(Atomic *atomic)
{
...
RpAtomicCallBackRender RpAtomicGetRenderCallBack(const RpAtomic * atomic) {
	return *PLUGINOFFSET(RpAtomicCallBackRender, atomic, re3RenderCBPluginOffset);
}

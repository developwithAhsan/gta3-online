...
cSampleManager::Initialise(void)
{
	...
	
	{
		...
		debug("AUDIO: add_providers() found %d provider(s), defaultProvider=%d\n", m_nNumberOfProviders, defaultProvider);
#endif
		if ( defaultProvider >= 0 && defaultProvider < m_nNumberOfProviders )
		{
#ifdef __EMSCRIPTEN__
			bool providerOk = set_new_provider(defaultProvider);
			debug("AUDIO: set_new_provider(%d) -> %s (ALDevice=%p ALContext=%p)\n", defaultProvider, providerOk ? "success" : "FAILED", (void*)ALDevice, (void*)ALContext);
#else
		...
		{
#ifdef __EMSCRIPTEN__
			debug("AUDIO: FAILED -- no provider available (defaultProvider=%d, m_nNumberOfProviders=%d), cSampleManager::Initialise returning false\n", defaultProvider, m_nNumberOfProviders);
#endif
			Terminate();
			return false;
		}

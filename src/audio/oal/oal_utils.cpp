...
// Emscripten's OpenAL implementation doesn't support the EFX extension and
// doesn't define alGetProcAddress. Every EFX/EAX code path in this codebase
// is gated behind alcIsExtensionPresent(ALC_EXT_EFX_NAME), which Emscripten
// always reports as false, so this stub is never actually exercised -- it
// only needs to exist to satisfy the linker.
extern "C" void *alGetProcAddress(const ALchar *fname)
{
	return nullptr;
}
#endif
